...
<script type="text/javascript">
	function replyOnSuggest(art_id) {
		$.ajax({
			url: '${pageContext.request.contextPath}/admin/suggest/updateProc',
			type: 'post',
			data: JSON.stringify({ 'art_id': art_id, 'rep_content': $('.suggest-reply').val() }),
			dataType: 'json',
	...
	$('.delete-reply-button').on('click', e => {
		...
		$.ajax({
			url: '${pageContext.request.contextPath}/admin/suggest/delete',
			type: 'post',
			data: JSON.stringify({ 'art_id': art_id, 'rep_id': rep_id }),
			dataType: 'json',
			contentType: 'application/json',
			success: data => {
...
<div class="admin-side-container">
	...
	<form method="post" onsubmit="return replyOnSuggest(${article.art_id});">
		<div class="display-flex flex-direction-column justify-content-flex-start align-items-stretch padding-10px" style="border-radius: 5px; background-color: rgba(var(--subtheme-rgb), 0.25);">
			<div class="display-flex justify-content-space-between align-items-center">
				<h3 class="color-subtheme margin-0 padding-10px">답글 달기</h3>
				<button type="submit" class="theme-button margin-10px" style="padding: 5px 10px;">등록</button>
			</div>
			<textarea class="suggest-reply background-color-theme margin-10px padding-custom2 overflow-y-auto" name="rep_content" style="outline: none; border: 1px solid var(--subtheme); border-radius: 5px; height: 160px;" maxlength="500" required></textarea>
		</div>
	</form>
