	...
	<main>
		...
		<div class="container display-flex justify-content-center align-items-center full-height">
			<div class="display-flex flex-direction-column justify-content-center align-items-center" style="border: 2.5px solid var(--subtheme); border-radius: 10px; padding: 20px;">
				<span>아이디 검색 결과</span>
				<span class="color-subtheme font-size-20px font-weight-bolder margin-10px">
					<c:set var="len" value="${fn:length(username) }"/>
					<fmt:formatNumber var="half" value="${len / 2 }" pattern="0"/>
					${fn:substring(username, 0, len - half) }
					<c:forEach var="i" begin="1" end="${half }">*</c:forEach>
				</span>
				<button class="subtheme-button" style="padding: 5px 10px;" onclick="location.href='${pageContext.request.contextPath}/';">메인으로 돌아가기</button>
			</div>
		</div>
		<!-- 각자 페이지 들어갈 공간 끝 -->
