...
<head>
<meta charset="UTF-8">
<title><c:choose><c:when test="${type == 'password/resetProc' }">비밀번호 재설정</c:when><c:when test="${type == 'id/searchResult' }">아이디 찾기</c:when></c:choose> ▒ ShareGo</title>
...
<body class="full-height">
	...
	<main>
		<!-- 각자 페이지 들어갈 공간 시작 -->
		<div class="container display-flex justify-content-center align-items-center full-height">
			<div class="display-flex flex-direction-column justify-content-center align-items-center" style="border: 2.5px solid var(--subtheme); border-radius: 10px; padding: 20px;">
				<h2 class="color-subtheme align-self-flex-start">
					<c:choose>
						<c:when test="${type == 'password/resetProc' }">비밀번호 재설정</c:when>
						<c:when test="${type == 'id/searchResult' }">아이디 찾기</c:when>
					</c:choose>
				</h2>
				<form name="frm" action="${pageContext.request.contextPath }/${type}" method="post">
					<div class="display-flex flex-direction-column justify-content-flex-start align-items-center">
						<div class="display-flex justify-content-flex-start align-items-center margin-10px margin-hor-0">
							<label for="name" class="font-size-16px font-weight-bolder margin-right-10px width-50px">이름</label>
							<input type="text" class="font-size-16px" name="name" style="min-width: 0; max-width: none; width: 240px;" autofocus required>
						</div>
						<div class="display-flex justify-content-flex-start align-items-center margin-10px margin-hor-0">
							<label for="email" class="font-size-16px font-weight-bolder margin-right-10px width-50px">이메일</label>
							<input type="email" class="font-size-16px" name="email" style="min-width: 0; max-width: none; width: 240px;" required>
						</div>
						<button type="submit" class="subtheme-button font-size-18px align-self-stretch" style="padding: 5px 20px; margin: 20px 0;">확인</button>
					</div>
				</form>
			</div>
		</div>
