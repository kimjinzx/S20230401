...
<div id="usernav">
	...
	<main>
		<div class="container padding-10px">
			<div class="article-view">
				<div class="board-title margin-50px margin-hor-0" align="center" style="border: 1px solid transparent; border-radius: 20px; background-color: rgba(var(--subtheme-rgb), 0.25)">
					<h1 class="color-subtheme text-align-left padding-10px padding-hor-20px">${categoryName} 게시판</h1>
					<p class="translucent-theme-font text-align-left padding-10px padding-hor-20px" style="padding-top: 0;">혼자서는 힘들어요ㅠㅠ 같이 할 사람을 찾아보세요</p>
				</div>
				...
				<div class="board-category display-flex justify-content-flex-start font-size-16px font-weight-bolder" align="center">
					<c:forEach var="comm" items="${categories}">
						<div class="item full-width" style="border-bottom: 2px solid var(--subtheme); ${comm.comm_id == category? 'border: 2px solid var(--subtheme); border-bottom: 0px;':''}">
							<a href="${pageContext.request.contextPath}/board/together?category=${comm.comm_id}" ${comm.comm_id != category ? 'style="color: rgba(var(--theme-font-rgb), 0.5);"' : '' } class="active full-width full-height display-block padding-5px padding-hor-0">${comm.comm_id == 1000 ? '전체' : comm.comm_value}</a>
						</div>
					</c:forEach>
				</div>
							...
							<div class="view-inner" style="display: flex; flex-direction: column; justify-content: center; flex-grow: 1;">
								...
								<div class="view-top flex-grow-1 display-flex justify-content-space-between align-items-center">
									<div class="flex-grow-1 display-flex justify-content-flex-start align-items-center">
										<c:if test="${article.brd_id != category }">
											<span class="category-name color-subtheme font-size-12px font-weight-bold display-flex justify-content-center align-items-center margin-right-5px" style="border-radius: 2.5px; border: 2px solid var(--subtheme); padding: 1.25px 2.5px;">${article.c3_comm_value}</span>
										</c:if>
										<span class="article-title span-ellipsis font-weight-bolder"><a href="${pageContext.request.contextPath}/board/together/${article.art_id}?brd_id=${article.brd_id}&category=${category}">${article.art_title}</a></span>
									</div>
								...
								<div class="view-middle display-flex justify-content-space-between align-items-center padding-5px padding-hor-0">
									...
									<div class="display-flex justify-content-flex-end align-items-center">
										<c:if test="${article.c1_comm_value != null}">
											<button class="btn margin-right-5px font-weight-bolder" ${article.trd_status == 401 ? 'style="background-color: var(--subtheme);"' : '' }>${article.c1_comm_value}</button>
										</c:if>
										<c:choose>
											<c:when test="${article.trd_cost == null || article.trd_cost == 0}">
												<!-- <span class="font-size-20px font-weight-bolder color-subtheme">무료</span> -->
												<span class="font-size-20px font-weight-bolder color-subtheme"/>
											</c:when>
											<c:when test="${article.trd_cost != null && article.trd_cost != 0}">
												<span class="font-size-20px font-weight-bolder color-theme-font"><fmt:formatNumber value="${article.trd_cost}" pattern="###,###,###,###,###"/> 원</span>
											</c:when>
										</c:choose>
									</div>
								</div>
									...
									<div class="display-flex justify-content-space-between align-items-center">
										...
										<div class="display-flex justify-content-flex-end align-items-center">
											<div class="display-flex justify-content-flex-start align-items-center margin-hor-2_5px">
												...
												<span class="margin-hor-2_5px">${article.repCount == null ? 0 : article.repCount}</span>
											</div>
											<div class="display-flex justify-content-flex-start align-items-center margin-hor-2_5px">
												<!-- https://ionic.io/ionicons -->
		...
		<!-- 					<option value="article">제목+내용</option>
							...
							<input type="text" name="keyword" placeholder="검색어를 입력하세요" required="required">
	        				<button type="submit" class="adv-hover">검색 </button><p>
					</form>
				</div>
				<!-- 페이징 -->
				<div class="board_paging" align="center" style="font-size: 18px; clear: both;">
					<button class="paging-block display-flex justify-content-center align-items-center" 	onclick="location.href='${pageContext.request.contextPath }/board/together?currentPage=${page.startPage-page.pageBlock }&category=${category}${param.search == null ? '' : '&search='.concat(param.search) }${param.keyword == null ? '' : '&keyword='.concat(param.keyword) }';" ${page.startPage <= page.pageBlock ? 'disabled' : '' }>
						<svg viewBox="0 0 256 512">
							<path d="M 224 32 L 32 256 224 480"/>
						</svg>
					</button>
					<c:forEach var="i" begin="${page.startPage }" end="${page.endPage }">
						<button class="paging-page display-flex justify-content-center align-items-center" 	onclick="location.href='${pageContext.request.contextPath }/board/together?currentPage=${i }&category=${category}${param.search == null ? '' : '&search='.concat(param.search) }${param.keyword == null ? '' : '&keyword='.concat(param.keyword) }';" ${page.currentPage == i ? 'disabled' : '' }>
							<span>${i }</span>
						</button>
					</c:forEach>
					<button class="paging-block display-flex justify-content-center align-items-center" 	onclick="location.href='${pageContext.request.contextPath }/board/together?currentPage=${page.endPage + 1 }&category=${category}${param.search == null ? '' : '&search='.concat(param.search) }${param.keyword == null ? '' : '&keyword='.concat(param.keyword) }';" ${page.endPage == page.totalPage ? 'disabled' : '' }>
						<svg viewBox="0 0 256 512">
